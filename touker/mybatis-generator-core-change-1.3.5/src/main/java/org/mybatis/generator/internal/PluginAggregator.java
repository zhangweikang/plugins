...
public final class PluginAggregator implements Plugin {
    ...
    @Override
    public boolean sqlMapQueryParamElementGenerated(XmlElement sqlXmlElement, IntrospectedTable introspectedTable) {
        boolean rc = true;

        for (Plugin plugin : plugins) {
            if (!plugin.sqlMapQueryParamElementGenerated(sqlXmlElement,
                    introspectedTable)) {
                rc = false;
                break;
            }
    ...
    @Override
    public boolean sqlMapFindSelectiveElementGenerated(XmlElement sqlXmlElement, IntrospectedTable introspectedTable) {
        boolean rc = true;

        for (Plugin plugin : plugins) {
            if (!plugin.sqlMapFindSelectiveElementGenerated(sqlXmlElement,
                    introspectedTable)) {
                rc = false;
                break;
            }
